...
export class ReportadorAST {
  private contadorNodos = 0;
  private lineas: string[] = [];

  // entrada

  generar(ast: any): string {
  ...
  private visitarNodo(nodo: any, idPadre: string | null): string {
    ...
    // Es un nodo del AST
    const tipoNodo: string = nodo.type ?? 'Desconocido';
    const id = this.nuevoId();
  ...
  private visitarHijos(nodo: any, idPadre: string): void {
    const t = nodo.type;

    switch (t) {
      case 'Program':
        for (const d of (nodo.declarations ?? [])) this.visitarNodo(d, idPadre);
        break;
      ...
      case 'Block':
        for (const s of (nodo.statements ?? [])) this.visitarNodo(s, idPadre);
        break;

      case 'VarDecl':
        if (nodo.value) this.visitarNodo(nodo.value, idPadre);
        break;
      ...
      case 'FunctionCall':
        for (const a of (nodo.args ?? [])) this.visitarNodo(a, idPadre);
        break;

      case 'IfStatement':
        this.visitarNodo(nodo.condition,  idPadre);
        this.visitarNodo(nodo.thenBlock,  idPadre);
        if (nodo.elseClause) this.visitarNodo(nodo.elseClause, idPadre);
        break;

      case 'ForStatement':
        if (nodo.form === 'classic') {
          if (nodo.init)      this.visitarNodo(nodo.init,      idPadre);
          if (nodo.condition) this.visitarNodo(nodo.condition, idPadre);
          if (nodo.post)      this.visitarNodo(nodo.post,      idPadre);
      ...
      case 'SwitchStatement':
        ...
        for (const c of (nodo.cases ?? [])) {
          ...
          for (const s of (c.statements ?? [])) this.visitarNodo(s, cid);
        }
        if (nodo.defaultCase) {
          const did = this.nodoSintetico('default', idPadre, '#21262d', '#f0883e');
          for (const s of (nodo.defaultCase.statements ?? [])) this.visitarNodo(s, did);
        }
        break;
      ...
      case 'SliceLiteral':
        for (const e of (nodo.elements ?? [])) this.visitarNodo(e, idPadre);
        break;

      case 'SliceLiteral2D':
        for (const fila of (nodo.rows ?? [])) {
          const fid = this.nodoSintetico('fila', idPadre, '#21262d', '#58a6ff');
          for (const e of fila) this.visitarNodo(e, fid);
        }
        break;
      ...
      default:
        for (const [clave, val] of Object.entries(nodo)) {
          if (['type', 'line', 'column'].includes(clave)) continue;
  ...
  private etiquetaParaNodo(nodo: any): string {
    const t = nodo.type ?? '?';
    switch (t) {
  ...
  private colorParaTipo(t: string): { colorRelleno: string; colorTexto: string } {
    switch (t) {
  ...
  private nodoSintetico(
    etiqueta: string, idPadre: string,
    colorRelleno: string, colorTexto: string
  ): string {
    const id = this.nuevoId();
